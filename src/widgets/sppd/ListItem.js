...
const SppdListItem = ({ item, isImport = false, editable = false, onEdit, handleCheckChange, isSelected, hiddenSelectbox = true }) => {
          ...
          <CardBody className="align-self-center d-flex flex-column flex-md-row justify-content-between min-width-zero align-items-md-center">
            {isImport ? (
              ...
              <NavLink
                ...
                to={{
                  pathname: `${adminRoot}/rka/beban/biaya-operasional/sppd/${item.status === "REPORTED" ? "detail" : "add"}`,
                  state: { sppd_id: item.sppd_id },
                }}
              >
                <i className="iconsminds-right-1 font-weight-bold" />
                {item.status === "APPROVED" ? "Buat Laporan" : item.sppd_no}
              </NavLink>
            )}
            ...
            <div
              ...
              <div className="custom-control custom-checkbox align-self-center mr-4 p-0 my-0">
                <CustomInput
                  ...
                  type="checkbox"
                  id={`check_${item.sppd_id}`}
                  checked={isSelected}
                  onChange={(event) =>
                    handleCheckChange(event, item.sppd_id)
                  }
                  label=""
                />
              </div>
                ...
                <Form onSubmit={onEdit}>
                  <FormGroup>
                    ...
                    <Input
                      ...
                      value={newValue.sppd_name}
                      onChange={(e) =>
                        setNewValue({
                          ...newValue,
                          sppd_name: e.target.value,
                        })
                      }
                    />
                  </FormGroup>
                  <FormGroup>
                    ...
                    <CurrencyFormat
                      ...
                      value={newValue.sppd_value}
                      onValueChange={(e) => {
                        const { value } = e;
                        setNewValue({
                          ...newValue,
                          sppd_value: parseInt(value) ? parseInt(value) : 0,
                        });
                      }}
                    />
                  </FormGroup>
