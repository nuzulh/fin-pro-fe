...
import { addSppdItem } from "redux/actions";
...
const SppdForm = ({ item, addSppdItemAction, details }) => {
  ...
  const [data, setData] = useState({
    ...
    budget_plan_name: "",
    budget_plan_total: 0,
    travel_order_total: 0,
    ppn: 0,
    pph: 0,
  });
  const [sppdNo, setSppdNo] = useState({
    no: "",
    code: "",
    month: "",
  ...
  useEffect(() => {
    ...
    const travel_order_total = ppnValue + pphValue + total;

    if (travel_order_total > item.budget_plan.total) {
      setErrorMsg("Nilai yang diajukan melebihi nilai RAB!");
    } else {
      setData({ ...data, ppn: ppnValue, pph: pphValue, travel_order_total });
    }
  ...
  useEffect(() => {
    setData({
      ...data,
      travel_order_no: `${sppdNo.no}/SPPD/CDB-${sppdNo.code}/${sppdNo.month}/${sppdNo.year}`,
    });
  }, [sppdNo]);
  ...
  const onSubmit = async (e) => {
    ...
    const formData = {
      ...
      travel_order_name: item
        ? item.travel_order_name + `~~${item.travel_order_id}`
        : data.travel_order_name,
      budget_plan_id: item
    ...
    await axios
      .post(`${servicePath2}/travel-order-detail`, details)
      .then((res) => {
        addSppdItemAction([formData, history]);
      });
  };
  ...
  useEffect(() => {
    if (item) {
      ...
      if (temp) {
        const x = temp.filter((i) => i.evidence === "SPPD")[0];
        setNilai(x.price);
      }
    }
  }, [item]);
      ...
      <Form onSubmit={onSubmit}>
        <Row className="mx-1 mb-3 d-flex">
          ...
          <Input
            ...
            value={sppdNo.no}
            onChange={(e) => setSppdNo({ ...sppdNo, no: e.target.value })}
          />
          <h6 className="my-auto">/SPPD/CDB-</h6>
          <Input
            required
            className="p-1 mx-2"
            style={{
            ...
            value={sppdNo.code}
            onChange={(e) => setSppdNo({ ...sppdNo, code: e.target.value })}
          />
          ...
          <Input
            ...
            placeholder="Bulan"
            value={sppdNo.month}
            onChange={(e) => setSppdNo({ ...sppdNo, month: e.target.value })}
          />
          <h6 className="my-auto">/</h6>
          <Input
            required
            ...
            placeholder="Tahun"
            value={sppdNo.year}
            onChange={(e) => setSppdNo({ ...sppdNo, year: e.target.value })}
          />
        </Row>
        <Card className="mb-4">
          <CardBody>
            ...
            {showDetail && (
              <>
                <FormGroup className="mt-3">
                  ...
                  {item ? (
                    <p className="px-3">{item.travel_order_name}</p>
                  ) : (
                    <Input
                      required
                      className="rounded-lg"
                      value={data.travel_order_name}
                      onChange={(e) =>
                        setData({ ...data, travel_order_name: e.target.value })
                      }
                    />
                  )}
                </FormGroup>
        ...
        <Card className="mb-4">
          <CardBody>
            <h5 className="font-weight-bold text-center mb-4">Rincian SPPD</h5>
