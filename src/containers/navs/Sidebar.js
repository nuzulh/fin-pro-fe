...
class Sidebar extends Component {
  ...
  setSelectedLiActive = (callback) => {
    ...
    const oldliSubs = document.querySelector(".forth-level-menu  li.active");
    if (oldliSubs != null) {
      oldliSubs.classList.remove("active");
    }

    /* set selected parent menu */
    const selectedSublink = document.querySelector(
      ".forth-level-menu  a.active"
    );
  ...
  toggleSubMenuCollapse = (e, menuKey) => {
    e.preventDefault();
    const { collapsedSubMenus } = this.state;
    if (collapsedSubMenus.indexOf(menuKey) > -1) {
      this.setState({
        collapsedSubMenus: collapsedSubMenus.filter((x) => x !== menuKey),
      });
    } else {
      collapsedSubMenus.push(menuKey);
      this.setState({
        collapsedSubMenus,
      });
    }
                                          ...
                                          return (
                                            ...
                                            >
                                              {thirdSub.newWindow ? (
                                                ...
                                                thirdSub.subs.length > 0 ? (
                                                <>
                                                  <NavLink
                                                    className={`rotate-arrow-icon opacity-50 ${collapsedSubMenus.indexOf(
                                                  ...
                                                  <Collapse
                                                    isOpen={
                                                      collapsedSubMenus.indexOf(
                                                        `${item.id}_${index}_${thirdIndex}`
                                                      ) > -1
                                                    }
                                                  >
                                                    <Nav className="forth-level-menu">
                                                      {thirdSub.subs.map(
                                                        (four, fourIndex) => {
                                                          return (
                                                            <NavItem
                                                              key={`${item.id}_${index}_${fourIndex}`}
                                                            >
                                                              <NavLink
                                                                to={four.to}
                                                              >
